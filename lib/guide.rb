S_APP_ROOT = File.dirname(__FILE__)
#puts S_APP_ROOT
$:.unshift( File.join(S_APP_ROOT) )
require 'restaurant'
...
class Guide
  ...
  def get_action
    ...
    until Guide::Config.actions.include?(action)
      ...
      args   = user_response.downcase.strip.split(' ') # the first item in the argument array is our action
      action = args.shift
    end
    return action , args
  end
  ...
  def find(key="")
    ...
    if keyword[0]== '$'
      #puts "putting dollar sign was not needed.\n"
      keyword.slice!(0)
    end
